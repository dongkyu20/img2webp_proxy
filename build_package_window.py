...
def main():
    ...
    try:
        ...
        result = subprocess.run(
            ["pyinstaller", "-y", "img_proxy_window.spec"],
            check=True,
            capture_output=True,
            text=True
        )
        ...
        # 빌드 성공 확인
        if os.path.exists(os.path.join("dist", "img_proxy.exe")):
            print("\n빌드 성공!")
            print(f"패키지 위치: {os.path.abspath(os.path.join("dist", "img_proxy.exe"))}")
            ...
            print("\n패키지 내용:")
            for root, dirs, files in os.walk(os.path.join("dist", "img_proxy.exe")):
                level = root.replace(os.path.join("dist", "img_proxy.exe"), "").count(os.sep)
                indent = " " * 4 * level
